...
SekTheme::SekTheme(QObject *parent):QObject{parent}{
    connect(this,&SekTheme::darkModeChanged,this,[=]{
        Q_EMIT darkChanged();
    });
    connect(this,&SekTheme::darkChanged,this,[=]{refreshColors();});
    connect(this,&SekTheme::themeColorChanged,this,[=]{refreshColors();});
    themeColor(SekColors::getInstance()->Blue());
    darkMode(SekThemeType::DarkMode::Light);
    nativeText(false);
...
bool SekTheme::eventFilter(QObject *obj, QEvent *event){
    Q_UNUSED(obj);
...
QJsonArray SekTheme::awesomeList(const QString& keyword){
    QJsonArray arr;
    QMetaEnum enumType = Sekent_Awesome::staticMetaObject.enumerator(Sekent_Awesome::staticMetaObject.indexOfEnumerator("Sekent_AwesomeType"));
...
bool SekTheme::dark(){
    if(_darkMode == SekThemeType::DarkMode::Dark){
        return true;
    }else if(_darkMode == SekThemeType::DarkMode::Light){
        return false;
    }else if(_darkMode == SekThemeType::DarkMode::System){
        return _systemDark;
    }else{
        return false;
    }
