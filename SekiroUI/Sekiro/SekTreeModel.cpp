...
SekTreeModel::SekTreeModel(QObject *parent): QAbstractItemModel{parent}{
    dataSourceSize(0);
}

QModelIndex SekTreeModel::parent(const QModelIndex &child) const{
    return QModelIndex();
}

QModelIndex SekTreeModel::index(int row, int column,const QModelIndex &parent) const{
    if (!hasIndex(row, column, parent) || parent.isValid())
        return QModelIndex();
    return createIndex(row, column, _rows.at(row));
}

int SekTreeModel::rowCount(const QModelIndex &parent) const {
    return _rows.count();
};

int SekTreeModel::columnCount(const QModelIndex &parent) const {
    return 1;;
};

QVariant SekTreeModel::data(const QModelIndex &index, int role) const {
    switch (role) {
    case Qt::DisplayRole:
...
QHash<int, QByteArray> SekTreeModel::roleNames() const {
    return { {Qt::DisplayRole, "dataModel"} };
};

void SekTreeModel::setData(QList<Node*> data){
    beginResetModel();
    _rows = data;
    endResetModel();
}

void SekTreeModel::removeRows(int row,int count){
...
void SekTreeModel::insertRows(int row,QList<Node*> data){
...
QObject* SekTreeModel::getRow(int row){
    return _rows.at(row);
}

void SekTreeModel::checkRow(int row,bool chekced){
...
void SekTreeModel::setDataSource(QList<QMap<QString,QVariant>> data){
    _dataSource.clear();
    if(_root){
        delete _root;
        _root = nullptr;
...
void SekTreeModel::collapse(int row){
...
void SekTreeModel::expand(int row){
    if(_rows.at(row)->_isExpanded){
        return;
    }
...
void SekTreeModel::dragAnddrop(int dragIndex,int dropIndex,bool isDropTopArea){
...
bool SekTreeModel::hitHasChildrenExpanded(int row){
    auto itemData = _rows.at(row);
...
void SekTreeModel::refreshNode(int row){
    Q_EMIT dataChanged(index(row,0),index(row,0));
};

Node* SekTreeModel::getNode(int row){
    return _rows.at(row);
}

void SekTreeModel::allExpand(){
    beginResetModel();
    QList<Node*> data;
