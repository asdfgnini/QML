...
FramelessEventFilter::FramelessEventFilter(SekFramelessHelper* helper){
    _helper = helper;
...
bool FramelessEventFilter::nativeEventFilter(const QByteArray &eventType, void *message, QT_NATIVE_EVENT_RESULT_TYPE *result){
    ...
    if(uMsg == WM_WINDOWPOSCHANGING){
        ...
        *result = WVR_REDRAW;
        ...
        if(SekTools::getInstance()->isWindows11OrGreater() && _helper->hoverMaxBtn() && _helper->resizeable()){
            if (*result == HTNOWHERE) {
                *result = HTZOOM;
            }
            return true;
...
SekFramelessHelper::SekFramelessHelper(QObject *parent)
    : QObject{parent}
{

}

void SekFramelessHelper::classBegin(){
}

void SekFramelessHelper::_updateCursor(int edges){
    switch (edges) {
    case 0:
...
bool SekFramelessHelper::eventFilter(QObject *obj, QEvent *ev){
...
void SekFramelessHelper::componentComplete(){
    auto o = parent();
    do {
...
void SekFramelessHelper::_onScreenChanged(){
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window->winId());
...
void SekFramelessHelper::showSystemMenu(){
#ifdef Q_OS_WIN
...
void SekFramelessHelper::_onStayTopChange(){
...
SekFramelessHelper::~SekFramelessHelper(){
    if (!window.isNull()) {
...
bool SekFramelessHelper::hoverMaxBtn(){
...
QObject* SekFramelessHelper::maximizeButton(){
    QVariant appBar = window->property("appBar");
    if(appBar.isNull()){
        return nullptr;
    }
...
void SekFramelessHelper::setOriginalPos(QVariant pos){
    _originalPos.write(pos);
}

bool SekFramelessHelper::resizeable(){
    return !_fixSize.read().toBool();
}

bool SekFramelessHelper::_maximized(){
    return window->visibility() == QWindow::Maximized;
}

bool SekFramelessHelper::_fullScreen(){
