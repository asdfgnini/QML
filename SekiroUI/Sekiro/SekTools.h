...
/**
 * @brief The SekTools class
 */
class SekTools : public QObject
{
    Q_OBJECT
    QML_NAMED_ELEMENT(SekTools)
    QML_SINGLETON
private:
    explicit SekTools(QObject *parent = nullptr);
public:
    SINGLETON(SekTools)
    static SekTools *create(QQmlEngine *qmlEngine, QJSEngine *jsEngine){return getInstance();}
